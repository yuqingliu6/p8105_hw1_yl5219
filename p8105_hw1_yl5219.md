...
variable in this dataset are `temp` (temperature), `humid` (humidity),
and `time_hour`. The value of `temp` (temperature) ranges from 24.08 to
57.92. The mean temperature is 39.58. The value of `humid` (humidity)
ranges from 32.86 to 100.00. The mean humidity is 65.48. The value of
`time_hour` ranges from 2013-01-01 01:00:00 to 2013-01-15 23:00:00. The
mean humidity is 2013-01-08 12:28:09.
...
vector (`vec_char`) and factor vector (`vec_factor`) did not work. This
is because the variable in the random sample (`norm_samp`) is numeric
(aka contain numbers). Among the logical vector, TRUE is treated as the
number 1, and FALSE is treated as number 0. We can only do mean
calcualtion for variable that include numbers.
